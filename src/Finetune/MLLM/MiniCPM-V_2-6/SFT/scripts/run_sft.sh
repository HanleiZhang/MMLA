...
WORK_PATH="$BASE_PATH/MMLA/src/Finetune/MLLM/MiniCPM-V_2-6/SFT"
...
for dataset in "${!task_map[@]}"; do
    ...
    RUN_NAME="${DATASET_NAME}-${TASK_NAME}"
    MODEL_PATH="$BASE_PATH/models_sft/$DATASET_NAME/$TASK_NAME/${MODEL_NAME}_SFT/${RUN_NAME}"
    ...
    CUDA_VISIBLE_DEVICES=${DEVICE_IDS} NPROC_PER_NODE=$nproc_per_node ${CONDA_PATH} run -n ${ENV_NAME} --no-capture-output swift sft \
        ...
        --train_type lora \
        --dataset $FRAMEWORK_PATH/video_prompt_data/$DATASET_NAME/train.jsonl \
        --val_dataset $FRAMEWORK_PATH/video_prompt_data/$DATASET_NAME/dev.jsonl \
        --deepspeed zero2 \
        --max_length 1024 \
        --lora_rank 16 \
        --lora_alpha 32 \
